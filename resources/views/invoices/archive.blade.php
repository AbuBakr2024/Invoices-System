...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">الفواتير</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ قائمه الفواتير المتارشفه</span>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
                    ...
                    <table class="table text-md-nowrap" id="example1" data-page-length='50'>
                        <thead>
                            <tr>
                                ...
                                <th class="wd-25p border-bottom-0" colspan="2" >العمليات</th>
                        ...
                        <a href="{{route('edit.invoicesdetails',$item->id)}}">{{ $item->section->section_name }}</a>
                             ...
                             @endif
                                ...
                                @endcan
                                <td> <a class="btn btn-info" href="#" data-invoice_id="{{ $item->id }}"
                                    data-toggle="modal" data-target="#move_invoice">
                                   نقل الى قائمه الفواتير
                                </a>
                                </td>
...
<div class="modal-dialog" role="document">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <form action="{{ route('destroyarchive.invoices', $item->id) }}" method="post">
                {{ method_field('delete') }}
                {{ csrf_field() }}
        </div>
...
{{-- move model --}}
    <div class="modal fade" id="move_invoice" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">الغاء ارشفه الفاتورة</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
                <form action="{{ route('move.invoices', $item->id) }}" method="post">
                    {{ csrf_field() }}
            </div>
            <div class="modal-body">
                هل انت متاكد من عملية الغاء الارشفه ؟
                <input type="hidden" name="invoice_id" id="invoice_id" value="">
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">الغاء</button>
...
{{-- move --}}
<script>
    $('#move_invoice').on('show.bs.modal', function(event) {
        var button = $(event.relatedTarget)
        var invoice_id = button.data('invoice_id')
        var modal = $(this)
        modal.find('.modal-body #invoice_id').val(invoice_id);
